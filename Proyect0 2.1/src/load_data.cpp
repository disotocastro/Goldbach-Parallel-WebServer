...
int LoadData(std::string file_name) {
  ...
  while (std::getline(file, line)) {
    ...
    iss >> sim.file_name >> sim.duration >> sim.thermal_diffusivity >>
        sim.size >> sim.sensitivity;
  ...
  for (size_t i = 0; i < simulations[0].matrix->rows; i++){
    for (size_t j = 0; j < simulations[0].matrix->cols; j++)
    {
...
Matrix* read_matrix_from_file(const std::string& file_name) {
  ...
  for (unsigned long long i = 0; i < matrix->rows; ++i) {
    file.read(reinterpret_cast<char*>(matrix->data[i]), matrix->cols * sizeof(double));
  }
