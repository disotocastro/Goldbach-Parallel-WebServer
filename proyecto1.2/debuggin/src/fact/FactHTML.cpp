...
void FactHTML::consume(std::vector<FactNumber*> data_resolved) {
  ...
  data_resolved[0]->httpResponse.setHeader("Content-type",
...
void FactHTML::sendSuccessResponse(std::vector<FactNumber*> data_resolved,
  ...
  data_resolved[0]->httpResponse.body()
      << "<!DOCTYPE html>\n"
      << "<html lang=\"en\">\n"
      << "  <meta charset=\"ascii\"/>\n"
      << "  <title>" << title << "</title>\n"
  ...
  for (size_t i = 0; i < data_resolved.size(); i++) {
    std::string numero = std::to_string(data_resolved[i]->numero);
    std::string resultado = results[i];
    data_resolved[0]->httpResponse.body()
        << "  <h2 class=\"blue\">" << numero << ": <span class=\"small\">"
        << resultado << "</span></h2>\n";
  }
...
std::vector<std::string> FactHTML::FactorizeToString(
    std::vector<FactNumber*> generalFactors) {
