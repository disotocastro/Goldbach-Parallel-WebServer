...
std::string HttpDispatcher::extractKey(const RequestResponseStruct& package) const {
  // Iterar sobre las colas registradas
  for ( const auto& pair : this->toQueues ) {
   // Verificar si el URI de la solicitud HTTP entrante comienza con la clave de
    // la cola actual
 ...
 // Si no se encuentra coincidencia, devolver una cadena vacía  return "";
}
...
void HttpDispatcher::consume(RequestResponseStruct data) {
  ...
  if ( iterador == this->toQueues.end() ) {
    // Si la clave no está en el mapa, enviar una respuesta 404 Not Found al cliente
...
void HttpDispatcher::serverNotFound(RequestResponseStruct& data) const {
  ...
  data.httpResponse->setHeader("Content-type"
